...
#m_from=(m1) # specify model names that you generate data from - has to be of the m1, m2,... form
#m_with=(m1)# specify model names that you fit data the generated data with - has to be of the m1, m2,... form
Nsubj=30 #specify how many subjects you want to simulate data for
sIterFrom=51 #specify the first of the number of elements in the job array on hpc
sIterTo=51 #specify the last of the number of elements in the job array on hpc
modelNo=${#m_from[@]}
...
stanSpec=mp_rec_spec_test; # load stan spec
...
do
	...
	echo $model_from
	echo $model_with
	sbatch --array=${sIterFrom}-${sIterTo} --job-name=F${model_from}_W${model_with}_${g}_N$Nsubj t2/slrm_test_mp_recovery.slurm $model_from $model_with $g $Nsubj $stanSpec
