...
sIterFrom=1 #specify the first of the number of elements in the job array on hpc
sIterTo=25 #specify the last of the number of elements in the job array on hpc
modelNo=${#m_from[@]}
...
do
	...
	echo $model_from
	echo $model_with
	# sbatch --array=${sIterFrom}-${sIterTo} --job-name=F${model_from}_W${model_with}_${g}_N$Nsubj slurm/mp_recovery_SL3.slurm $model_from $model_with $g $Nsubj $stanSpec
